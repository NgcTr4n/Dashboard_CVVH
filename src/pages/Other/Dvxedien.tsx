import React, { useEffect, useState } from "react";
import { useAppDispatch, useAppSelector } from "../../hooks";
import Layout from "../../layout";
import "../form.css";
import {
  deleteData,
  fetchDvxedien,
  uploadDataWithImage,
} from "../../features/dvxedienSlice";

const Dvxedien: React.FC = () => {
  const dispatch = useAppDispatch();
  const { dvxedien, loading, error } = useAppSelector(
    (state) => state.dvxedien
  );

  const [loaihinh, setLoaihinh] = useState<string>("");
  const [songuoi, setSonguoi] = useState<string>("");
  const [sotien, setSotien] = useState<string>("");

  const handleUpload = (e: React.FormEvent) => {
    e.preventDefault();
    if (loaihinh) {
      dispatch(
        uploadDataWithImage({
          loaihinh,
          songuoi,
          sotien,
        })
      );
      // Reset các giá trị input
      setLoaihinh("");
      setSonguoi("");
      setSotien("");
    }
  };

  useEffect(() => {
    dispatch(fetchDvxedien());
  }, [dispatch]);

  const handleDelete = (id: string) => {
    dispatch(deleteData(id)); // Gọi hàm deleteData
  };
  //   if (!lichsucontent || lichsucontent.length === 0) {
  //     return <div>No items found.</div>;
  //   }

  return (
    <Layout>
      <div className="app-container">
        <div className="form-container">
          <div>
            <h2 className="form-heading">Dịch vụ cho thuê</h2>
          </div>
          <form className="mt-8 space-y-6" onSubmit={handleUpload}>
            <div>
              <input
                id="loaihinh"
                name="loaihinh"
                type="text"
                required
                className="input-field"
                placeholder="Loại hình"
                value={loaihinh}
                onChange={(e) => setLoaihinh(e.target.value)}
              />
            </div>
            <div>
              <input
                id="songuoi"
                name="songuoi"
                type="text"
                required
                className="input-field"
                placeholder="Số người"
                value={songuoi}
                onChange={(e) => setSonguoi(e.target.value)}
              />
            </div>
            <div>
              <input
                id="sotien"
                name="sotien"
                type="text"
                required
                className="input-field"
                placeholder="Số tiền"
                value={sotien}
                onChange={(e) => setSotien(e.target.value)}
              />
            </div>

            <div>
              <button type="submit" disabled={!loaihinh} className="button">
                Add New
              </button>
            </div>
          </form>
        </div>
        <div className="table-container">
          <h2 className="table-heading">Thông tin bổ sung - Bảo mật</h2>
          <table className="data-table">
            <thead>
              <tr>
                <th>Loại hình</th>
                <th>Số người</th>
                <th>Số tiền</th>
                <th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {dvxedien.map((card) => (
                <tr key={card.id}>
                  <td>{card.loaihinh}</td>

                  <td>{card.songuoi}</td>
                  <td>{card.sotien}</td>
                  <td>
                    <button
                      onClick={() => handleDelete(card.id)} // Gọi hàm delete
                      className="button"
                    >
                      Delete
                    </button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        {error && <div className="error-message">{error}</div>}
        {loading && <div className="loading-message">Loading...</div>}
      </div>
    </Layout>
  );
};

export default Dvxedien;
